...
class IntensityMeasureCorrelationModel():
    ...
    def plot(self, xlabel=self.imts[0], ylabel=self.imts[1], title="Correlation model", cbar_title=None):
        """
        ...
        :return:
        """
        x = np.logscale(np.log10(self.bounds[0][0]), np.log10(self.bounds[0][1]), num=10)
        y = np.logscale(np.log10(self.bounds[1][0]), np.log10(self.bounds[1][1]), num=10)
        nx = len(x)
        ny = len(y)
